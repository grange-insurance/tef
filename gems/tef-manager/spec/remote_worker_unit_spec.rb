...
describe 'RemoteWorker, Unit' do
  ...
  let(:clazz) { TEF::Manager::RemoteWorker }
  ...
  describe 'class level' do
    ...
    it 'will complain if not provided a with a name' do
      ...
      expect { clazz.new(configuration) }.to raise_error(ArgumentError, /name.*must be provided/i)
    end
    ...
    it 'can be provided with a work queue upon creation' do
      configuration[:work_queue] = mock_queue
      worker = clazz.new(configuration)
    ...
    it 'will complain if not provided a with a work queue' do
      configuration.delete(:work_queue)

      expect { clazz.new(configuration) }.to raise_error(ArgumentError, /work.queue.*must be provided/i)
    end
    ...
    it 'will complain if not provided a with a type' do
      ...
      expect { clazz.new(configuration) }.to raise_error(ArgumentError, /type.*must be provided/i)
    end
    ...
    it 'can be provided with an update interval upon creation' do
      configuration[:update_interval] = 12345
      worker = clazz.new(configuration)
    ...
    it 'sets a default update interval of 30 seconds if one is not provided' do
      configuration.delete(:update_interval)
    ...
    it 'can be provided with a time limit upon creation' do
      configuration[:time_limit] = 12345
      worker = clazz.new(configuration)
    ...
    it 'sets a default time limit of 600 seconds if one is not provided' do
      configuration.delete(:time_limit)
    ...
    it 'can be provided with a resource manager upon creation' do
      configuration[:resource_manager] = :some_manager
      worker = clazz.new(configuration)
    ...
    it 'will complain if not provided a with a type' do
      ...
      expect { clazz.new(configuration) }.to raise_error(ArgumentError, /resource.manager.*must be provided/i)
    end
  ...
  describe 'instance level' do
    ...
    let(:fake_task) { FakeTask.new }

    let(:mock_logger) { create_mock_logger }
    let(:mock_queue) { create_mock_queue }
    let(:mock_resource_manager) { thing = double('mock resource manager')
                                  allow(thing).to receive(:add_ref).and_return(true)
                                  allow(thing).to receive(:remove_ref)
                                  thing }
    let(:configuration) { {logger: mock_logger,
                           name: 'test_worker',
                           work_queue: mock_queue,
    ...
    describe 'worker status' do
      ...
      it 'is does not stall if it not working a task (i.e. is idling)' do
        interval = 1
        configuration[:time_limit] = interval
        worker = clazz.new(configuration)
      ...
      it 'is considered missing if it has not been updated within the time span of its update interval' do
        interval = 1
        configuration[:update_interval] = interval
        worker = clazz.new(configuration)
